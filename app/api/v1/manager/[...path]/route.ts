...
const buildTargetUrl = ({
  ...
  return `${normalizeBaseUrl(apiBaseUrl)}/api/v1/manager/${normalizedPath}${search}`;
};
...
const proxyManagerRequest = async (
  request: NextRequest,
  params: { path: string[] },
) => {
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: `Unable to reach upstream manager endpoint (${targetUrl}). ${message}`,
      },
      { status: 502 },
    );
  }
