...
const buildTargetUrl = ({
  ...
  return `${normalizeBaseUrl(apiBaseUrl)}/api/v1/customer/${normalizedPath}${search}`;
};
...
const proxyCustomerRequest = async (
  request: NextRequest,
  params: { path: string[] },
) => {
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: `Unable to reach upstream customer endpoint (${targetUrl}). ${message}`,
      },
      { status: 502 },
    );
  }
...
export async function GET(
  ...
  return proxyCustomerRequest(request, params);
}
