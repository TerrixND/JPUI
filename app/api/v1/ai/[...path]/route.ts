...
const buildTargetUrl = ({
  ...
  return `${normalizeBaseUrl(apiBaseUrl)}/api/v1/ai/${normalizedPath}${search}`;
};
...
const proxyAiRequest = async (
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: `Unable to reach upstream AI endpoint (${targetUrl}). ${message}`,
      },
      { status: 502 },
    );
  }
...
export async function GET(
  ...
  return proxyAiRequest(request, params);
}
