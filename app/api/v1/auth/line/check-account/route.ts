...
export async function POST(request: NextRequest) {
  ...
  const targetUrl = `${normalizeBaseUrl(apiBaseUrl)}/api/v1/auth/line/check-account`;
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: `Unable to reach upstream LINE account check endpoint (${targetUrl}). ${message}`,
      },
      { status: 502 },
    );
  }
