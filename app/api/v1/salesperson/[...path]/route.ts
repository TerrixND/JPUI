...
const buildTargetUrl = ({
  ...
  return `${normalizeBaseUrl(apiBaseUrl)}/api/v1/salesperson/${normalizedPath}${search}`;
};
...
const proxySalespersonRequest = async (
  request: NextRequest,
  params: { path: string[] },
) => {
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: `Unable to reach upstream salesperson endpoint (${targetUrl}). ${message}`,
      },
      { status: 502 },
    );
  }
...
export async function GET(
  ...
  return proxySalespersonRequest(request, params);
}
