...
function statusStyle(status: string) {
  switch (status) {
    case "CONFIRMED": return "bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400";
...
export default function SalespersonDashboard() {
  ...
  const stats = [
    ...
    },
    {
      ...
      href: `${dashboardBasePath}/possessions`,
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700/60 transition-colors">
          <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700/60">
            <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">Upcoming Appointments</h2>
          </div>
            ...
            {upcomingAppointments.map((apt) => (
              <div key={apt.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{apt.customer}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{apt.date} at {apt.time} &middot; {apt.items} item{apt.items !== 1 ? "s" : ""}</p>
                  </div>
                </div>
