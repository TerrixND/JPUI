...
export default function AdminStaffRules() {
  ...
  const { isAdminActionBlocked, isMainAdmin } = useRole();
  ...
  const [formCommissionPriority, setFormCommissionPriority] = useState("");
  const [formCommissionNote, setFormCommissionNote] = useState("");
  ...
  const onCreateRule = async () => {
    ...
    if (formRole === "SALES" && formCommissionRate.trim()) {
      const rate = Number(formCommissionRate.trim());
      if (Number.isNaN(rate) || rate < 0 || rate > 100) {
        setCreateError("Commission rate must be a number between 0 and 100.");
        return;
      }
    }
    if (formRole === "SALES" && formCommissionPriority.trim()) {
      const priority = Number(formCommissionPriority.trim());
      if (!Number.isInteger(priority) || priority < 0 || priority > 1000) {
        setCreateError("Commission priority must be an integer between 0 and 1000.");
        return;
      }
    }
    ...
    try {
      ...
      if (formRole === "SALES" && formCommissionRate.trim()) {
        const commission: Record<string, unknown> = {
          rate: Number(formCommissionRate.trim()),
        };
        if (formCommissionPriority.trim()) {
          commission.priority = Number(formCommissionPriority.trim());
        }
        if (formCommissionNote.trim()) {
          commission.note = formCommissionNote.trim();
        }
        payload.permissions = { commission };
      }
  ...
  const onRevokeRule = async (ruleId: string) => {
    ...
    try {
      const accessToken = await getAccessToken();
      const response = await revokeAdminStaffRule({ accessToken, ruleId });
  ...
  const onDeleteRule = async (rule: StaffOnboardingRule) => {
    ...
    if (staffRuleManageBlocked) {
      setDeleteError(staffRuleManageTooltip);
      return;
    }
    ...
    try {
      const accessToken = await getAccessToken();
      const response = await deleteAdminStaffRule({ accessToken, ruleId: rule.id });
      if (response.approvalSubmitted) {
        setActionInfo(
          response.message ||
      ...
      {!loading && !error && rules.length > 0 && (
        <div className="space-y-3">
          {rules.map((rule) => {
            ...
            const canRevoke = isMainAdmin
              ? rule.status !== "REVOKED"
              : rule.status !== "REVOKED" && rule.status !== "CLAIMED";
            const canDelete = rule.status === "REVOKED";
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 mb-2">
                        ...
                        {rule.setAsPrimaryManager && (
                          <span className="inline-block px-2 py-0.5 rounded bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 text-[11px] font-medium">
                            Primary Manager
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button
                        ...
                        disabled={!canRevoke || isRevoking || isDeleting || staffRuleManageBlocked}
                        title={
                          staffRuleManageBlocked
                            ? staffRuleManageTooltip
                            : !canRevoke
                              ? "Claimed rules can only be revoked by main admin."
                              : undefined
                        }
                      ...
                      <button
                        ...
                        disabled={!canDelete || isDeleting || isRevoking || staffRuleManageBlocked}
                        title={
                          staffRuleManageBlocked
                            ? staffRuleManageTooltip
                            : !canDelete
                              ? "Delete is available only for revoked rules."
                              : undefined
                        }
          ...
          <div className="relative w-full max-w-lg mx-4 sm:mx-auto mt-16 sm:mt-0 bg-white dark:bg-gray-900 rounded-2xl shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              ...
              {formRole === "SALES" && (
                <div className="p-3 rounded-xl border border-gray-200 dark:border-gray-700/60 bg-gray-50/60 dark:bg-gray-800/50 space-y-3">
                  <p className="text-[11px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                    Commission Configuration <span className="text-gray-400 dark:text-gray-500 normal-case">(optional)</span>
                  </p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[11px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">
                      ...
                      <input
                        ...
                        className={inputClass}
                      />
                      <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1">0 – 100, required if any commission field is set.</p>
                    </div>
                    <div>
                      <label className="block text-[11px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">
                        Priority
                      ...
                      <input
                        ...
                        max={1000}
                        step={1}
                        value={formCommissionPriority}
                        onChange={(e) => setFormCommissionPriority(e.target.value)}
                        placeholder="e.g. 100"
                        className={inputClass}
                      />
                      <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1">Integer 0 – 1000</p>
                    </div>
                  </div>
                  <div>
                    <label className="block text-[11px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">
                    ...
                    <input
                      type="text"
                      value={formCommissionNote}
                      onChange={(e) => setFormCommissionNote(e.target.value)}
                      placeholder="e.g. Launch campaign rate"
                      className={inputClass}
                    />
                  </div>
                </div>
